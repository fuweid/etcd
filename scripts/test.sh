...
function markdown_you_pass {
  # FIXME(fuweid): ./CONTRIBUTING.md:## Get your pull request reviewed
...
function govet_shadow_pass {
  # FIXME(fuweid): we should ignore the generated packages?
  #
...
function unparam_pass {
  # FIXME(fuweid): transport/listener.go:129:60: newListenConfig - result 1 (error) is always nil
  run_for_modules generic_checker run_go_tool "mvdan.cc/unparam"
}

function staticcheck_pass {
  # FIXME(fuweid): we should upgrade pb or ignore the pb package
  #
...
function revive_pass {
  # FIXME(fuweid): etcdserverpb/raft_internal_stringer.go:15:1: should have a package comment
...
function unconvert_pass {
  # FIXME(fuweid): pb package should be filtered out.
...
function ineffassign_per_package {
  ...
  # FIXME(fuweid): ineffassign should work with package instead of files
...
function nakedret_pass {
  # FIXME(fuweid): nakedret should work with -set_exit_status
